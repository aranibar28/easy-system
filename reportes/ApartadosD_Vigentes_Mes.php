...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'PRODUCTOS APARTADOS VIGENTES  DEL MES '.$mes,0,0,'C');
...
// Page footer
   ...
   $mes = DateTime::createFromFormat('m/Y', $mes)->format('m-Y');
    ...
    $listado = $objApartado->Listar_Apartados_Detalle('MES',$mes,'',1);
    $totales = $objApartado->Listar_Apartados_Totales('MES',$mes,'',1);
...
try {
  // Instanciation of inherited class
    ...
    if (is_array($listado) || is_object($listado))
    {
      ...
      $pdf->Text(10,$get_Y + 10,'TOTAL DE PRODUCTOS APARTADOS : '.number_format($total, 2, '.', ','));
      $pdf->Text(250,$get_Y + 45,'PRECIOS EN : '.$moneda);
    }



   $pdf->Output('I','ApartadosD_Vigentes_'.$mes_actual.'_del_'.$ano.'.pdf');
