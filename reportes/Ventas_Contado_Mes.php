    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            "Septiembre","Octubre","Noviembre","Diciembre");
             ...
             $ano = substr($mes,3,4);
             $this->Cell(105,10,'VENTAS AL CONTADO DEL MES '.$mes,0,0,'C');
            // Line break
            $this->Ln(20);
        }
    }
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'MONTO TOTAL INGRESADO POR VENTAS ESTE MES : '.number_format($total, 2, '.', ','));
        $pdf->Text(10,$get_Y + 15,'PRECIOS EN : '.$moneda);
    }


    $pdf->Output('I','Ventas_Contado_'.$mes_actual.'_del_'.$ano.'.pdf');
