...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(98);
            // Title
            $this->Cell(105,10,'COMPRAS AL CREDITO DEL MES '.$mes,0,0,'C');
            // Line break
            $this->Ln(20);
        }
    }
...
// Page footer
    ...
    $listado = $objCompra->Listar_Compras('MES',$mes,'','',2);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'MONTO TOTAL EN COMPRAS AL CREDITO : '.number_format($total, 2, '.', ','));
          $pdf->Text(10,$get_Y + 15,'PRECIOS EN : '.$moneda);
    }


    $pdf->Output('I','Compras_Credito_'.$mes_actual.'_del_'.$ano.'.pdf');
