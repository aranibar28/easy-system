...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105,10,'VENTAS (DETALLE) AL CREDITO DEL MES '.$mes,0,0,'C');
            // Line break
            $this->Ln(20);
        }
    }
...
// Page footer
   ...
   $mes = DateTime::createFromFormat('m/Y', $mes)->format('m-Y');
    ...
    $objVenta =  new Venta();
    $listado = $objVenta->Listar_Ventas_Detalle('MES',$mes,'',2);
    $parametros = $objVenta->Ver_Moneda_Reporte();
    $totales = $objVenta->Listar_Ventas_Totales('MES',$mes,'',2);
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
      ...
      $pdf->Text(10,$get_Y + 20,'TOTAL DE IGV EN CREDITOS : '.number_format($total_iva, 2, '.', ','));
    ...
    $pdf->Output('I','Ventas_Credito_'.$mes_actual.'_del_'.$ano.'.pdf');
