    ...
    var filas=0;
      ...
      if(inventariable == 0){
                ...
                '<span class="text-muted">'+especificacion+'</span></td>';
                 ...
                 tr_add += '<select class="form-control select-precio-venta" style="width:80px">';
                 tr_add += '<option value="'+precios[0]+'">'+precios[0]+'</option>';
                 tr_add += '<option value="'+precios[1]+'">'+precios[1]+'</option>';
                 tr_add += '<option value="'+precios[2]+'">'+precios[2]+'</option>';
                 tr_add += '</select></td>';
                 tr_add += '<td align="center" class="excento-venta" data-precio0="'+excentos[0]+'" data-precio1="'+excentos[1]+'" data-precio2="'+excentos[2]+'">'+exento+'</td>';
                 ...
                 if(existe==false){
                   ...
                   $('.select-precio-venta').change(function(){
                     totales();
                   });
...
function openPopUp(){
  var pop = window.open('?View=Clientes', '_blank', 'location=yes,height=570,width=820,scrollbars=yes,status=yes');
  pop.onbeforeunload = function(){
    var urlprocess = 'web/ajax/listarcliente.php';
    $.ajax({
      type:'POST',
      url:urlprocess,
      dataType: 'json',
      success: function(result){
        ...
        result.map(function(item){
          $('#cbCliente').append('<option value = "'+item[0]+'">'+item[2]+'</option>');
        });
      }
    });
  }
  ...
  $("#buscar_producto").autocomplete({
    ...
    focus: function( event, ui ) {
       var tipo_precio = 1;
        ...
        var precios = [ui.item.precio_venta_mayoreo, ui.item.precio_venta, ui.item.precio_venta_3];
        var excentos = [0,0,0];
        var precio_venta = ui.item.precio_venta;
        var precio_venta_mayoreo = ui.item.precio_venta_mayoreo;
        var precio_venta_3 = ui.item.precio_venta_3;
          ...
          agregar_detalle(idproducto,producto,datos,precio_venta,0.00,stock,perecedero,inventariable,precios, excentos);
            ...
            if(exento == 0)
            {
              ...
              if(tipo_precio == 1)
              {
                ...
                agregar_detalle(idproducto,producto,datos,precio_venta_3,precio_venta_mayoreo,stock,perecedero,inventariable,precios, excentos);
              }
            ...
            $("#tbldetalle tbody tr").each(function (index)
                {
                    ...
                    $(this).children("td").each(function (index2)
                    {
                        switch (index2)
                        {
                            ...
                            case 3:  campo3 = $(this).find('.select-precio-venta').val();
...
function enviar_data(){
  ...
  var comprobanteName = $("#cbCompro>option:selected").text();
              ...
              if(data=="Validado"){
                ...
                $('#modal_iconified_cash').modal('toggle');
                  ...
                  swal({
                    ...
                    function(isConfirm){
                      if (isConfirm) {
                        window.open('reportes/'+(comprobanteName.trim()=== 'TICKET'?'TicketV.php':'Factura.php')+'?venta=""',
                        'win2',
