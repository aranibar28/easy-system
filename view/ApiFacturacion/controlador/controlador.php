...
function controlador($accion)
{
    ...
    switch ($accion) {
        ...
        case 'CONSULTA_DNI':
            ...
            $ws = "https://api.apis.net.pe/v1/dni?numero=$dni";
            $header = array();
        ...
        case 'ADD_PRODUCTO':
            ...
            $producto = $objCompartido->obtenerProducto($_POST['idproducto']);
            ...
            foreach ($carrito as $k => $v) {
                if($v['idproducto']==$_POST['idproducto']){
                    $item = $k;
                    $existe = true;
                    break;
                }
            ...
            foreach ($carrito as $K => $v) {
                if($v['codigoafectacion']=='10'){
                    $op_gravadas = $op_gravadas+$v['precio_venta']*$v['cantidad'];
                }

                if($v['codigoafectacion']=='20'){
                    $op_exoneradas = $op_exoneradas+$v['precio_venta']*$v['cantidad'];
                }

                if($v['codigoafectacion']=='30'){
                    $op_inafectas = $op_inafectas+$v['precio_venta']*$v['cantidad'];
                }												
            }
            ...
            foreach($carrito as $k=>$v){
                echo "<tr>";
                echo "<td>".$k."</td><td>".$v['cantidad']."</td><td>".$v['unidad']."</td><td>".$v['nombre']."</td><td>".$v['precio_venta']."</td><td>".($v['precio_venta']*$v['cantidad'])."</td>";
                echo "</tr>";
            }
        ...
        case 'GUARDAR_VENTA':
            ...
            foreach ($carrito as $k => $v){
                ...
                if($producto['codigoafectacion']==10){
                    $igv_detalle = $v['precio_venta']*$v['cantidad']*$igv_porcentaje;
                ...
                $itemx = array(
                    ...
                    'cantidad'			=> $v['cantidad'],
                    'valor_unitario'	=> $v['precio_venta'],
                    'precio_unitario'	=> $v['precio_venta']*$factor_porcentaje,
                    'tipo_precio'		=> $producto['tipo_precio'], //ya incluye igv
                    'igv'				=> $igv_detalle,
                    'porcentaje_igv'	=> $igv_porcentaje*100,
                    'valor_total'		=> $v['precio_venta']*$v['cantidad'],
                    'importe_total'		=> $v['precio_venta']*$v['cantidad']*$factor_porcentaje,
